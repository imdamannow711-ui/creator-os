/* DONE RITE Creator OS — One-Click Browser Executor v0.4
...
function waitRecorderState(recorder,eventName,timeout){
  return new Promise((resolve,reject)=>{
    let timer=null;
    const done=()=>{cleanup();resolve();};
    const fail=e=>{cleanup();reject(e&&e.error||new Error('Recorder '+eventName+' failed.'));};
    function cleanup(){recorder.removeEventListener(eventName,done);recorder.removeEventListener('error',fail);if(timer)clearTimeout(timer);}
    recorder.addEventListener(eventName,done,{once:true});recorder.addEventListener('error',fail,{once:true});
    timer=setTimeout(()=>{cleanup();reject(new Error('Timed out waiting for recorder '+eventName+'.'));},timeout||5000);
  });
}
async function loadPlayableVideo(file){
  ...
  try{if(video.readyState<1)await waitEvent(video,'loadedmetadata',12000);if(video.readyState<2)await waitEvent(video,'loadeddata',12000);return {video,url};}
  catch(err){release(url);throw err;}
}
function makeCanvasForVideo(video,maxLongEdge){
  const vw=video.videoWidth||1080,vh=video.videoHeight||1920,maxEdge=Math.max(vw,vh),limit=Math.max(360,Number(maxLongEdge||1280)),scale=maxEdge>limit?limit/maxEdge:1;
  const canvas=document.createElement('canvas');canvas.width=Math.max(2,Math.round(vw*scale));canvas.height=Math.max(2,Math.round(vh*scale));return canvas;
}
...
function drawOverlay(ctx,overlay,w,h){
  ...
  const fontSize=Math.max(26,Math.round(w*.058)),x=Math.round(w*.08),y=Math.round(h*.17),maxWidth=Math.round(w*.78);
  ...
  lines.forEach((line,i)=>{ctx.lineWidth=Math.max(3,Math.round(w*.005));ctx.strokeStyle='rgba(0,0,0,.92)';ctx.fillStyle='#fff';ctx.strokeText(line,x,y+i*gap);ctx.fillText(line,x,y+i*gap);});
}
function overlayForTime(recipe,t){
  ...
  if(!overlays.length)return null;if(overlays.length===1)return overlays[0];
...
async function seekVideo(video,time){
  ...
  if(Math.abs(video.currentTime-target)<.04)return;video.currentTime=target;await waitEvent(video,'seeked',10000);
}
...
async function previewFrame(file,recipe,options){
  ...
  try{
    const cuts=recipe&&Array.isArray(recipe.selectedCuts)?recipe.selectedCuts:[],first=cuts[0];
    ...
    const canvas=makeCanvasForVideo(video,options.maxLongEdge||1280),ctx=canvas.getContext('2d');fitContain(ctx,video,canvas.width,canvas.height);drawOverlay(ctx,overlayForTime(recipe,0),canvas.width,canvas.height);
...
function createRecorder(stream,mime,options){
  const full={mimeType:mime,videoBitsPerSecond:Number(options.videoBitsPerSecond||6000000),audioBitsPerSecond:Number(options.audioBitsPerSecond||128000)};
  try{return new MediaRecorder(stream,full);}catch(e){return new MediaRecorder(stream,{mimeType:mime});}
}
async function renderLocalVideo(file,recipe,options){
  options=options||{};
  const caps=supports(),mime=pickMime(),AudioCtx=window.AudioContext||window.webkitAudioContext;
  ...
  let audioCtx=null,loaded=null,video=null,url='',canvasStream=null,combined=null,recorder=null,outputUrl='';
  try{
    audioCtx=new AudioCtx();
    if(audioCtx.state!=='running')await audioCtx.resume();
    loaded=await loadPlayableVideo(file);video=loaded.video;url=loaded.url;
    const canvas=makeCanvasForVideo(video,options.maxLongEdge||1280),ctx=canvas.getContext('2d',{alpha:false});canvasStream=canvas.captureStream(Number(options.fps||30));
    const source=audioCtx.createMediaElementSource(video),gain=audioCtx.createGain(),dest=audioCtx.createMediaStreamDestination();gain.gain.value=1;source.connect(gain);gain.connect(dest);
    ...
    const cuts=recipe.selectedCuts.map(c=>({start:Number(c.start),end:Number(c.end)})).filter(c=>c.end>c.start);let baseElapsed=0;
    await seekVideo(video,cuts[0].start);fitContain(ctx,video,canvas.width,canvas.height);drawOverlay(ctx,overlayForTime(recipe,0),canvas.width,canvas.height);recorder.start(250);
    for(let i=0;i<cuts.length;i++){
      const cut=cuts[i];
      if(i>0){
        if(recorder.state==='recording'){const p=waitRecorderState(recorder,'pause',4000);recorder.pause();await p;}
        await seekVideo(video,cut.start);
        if(recorder.state==='paused'){const r=waitRecorderState(recorder,'resume',4000);recorder.resume();await r;}
      }
      await video.play();await drawUntil(video,ctx,canvas,cut,baseElapsed,recipe,options.onProgress);video.pause();baseElapsed+=cut.end-cut.start;
    }
    ...
    try{if(video){video.pause();video.removeAttribute('src');video.load();}}catch(e){}release(url);
