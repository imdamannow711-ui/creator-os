/* DONE RITE Creator OS — One-Click Browser Executor v0.6
...
async function ensureFrameReady(video,targetTime){
  ...
  try{
    ...
    await waitUntil(()=>video.readyState>=2,30000,'video frame data');
...
async function seekVideo(video,time){await ensureFrameReady(video,time);}
...
async function renderLocalBatch(files,recipe,options){
  ...
  try{
    ...
    await ensureFrameReady(active.video,Number(recipe.selectedCuts[0].start||0));
    ...
    async function switchSource(sourceIndex,startTime){
      ...
      if(currentSourceIndex===sourceIndex&&active){await ensureFrameReady(active.video,startTime||0);return;}
      ...
      active=await loadPlayableVideo(list[sourceIndex]);
      await ensureFrameReady(active.video,startTime||0);
    ...
    await switchSource(firstIndex,cuts[0].start);fitContain(ctx,active.video,canvas.width,canvas.height);drawOverlay(ctx,overlayForTime(recipe,0),canvas.width,canvas.height);recorder.start(250);
    for(let i=0;i<cuts.length;i++){
      const cut=cuts[i];
      if(i>0){if(recorder.state==='recording'){const p=waitRecorderState(recorder,'pause',5000);recorder.pause();await p;}await switchSource(cut.sourceIndex,cut.start);if(recorder.state==='paused'){const r=waitRecorderState(recorder,'resume',5000);recorder.resume();await r;}}
      else await ensureFrameReady(active.video,cut.start);
      active.video.muted=false;await active.video.play();await drawUntil(active.video,ctx,canvas,cut,baseElapsed,recipe,options.onProgress);active.video.pause();baseElapsed+=cut.end-cut.start;
    }
