/* DONE RITE Creator OS — One-Click Ad Editor engine v0.4 */
(function(){
'use strict';
const VERSION='0.4';
...
function hookShot(angle,product,feature){const a=clean(angle).toLowerCase();if(/result|outcome/.test(a))return `Start on ${product} already set up or already working. Show the finished/useful state immediately, then move into the action.`;if(/demo|hands-on|utility|mid-action/.test(a))return `Start with your hands already using ${product}. No reach-in or intro. Make ${feature} visible in the first 1–2 seconds.`;if(/detail|curiosity|discovery|opinion|probing/.test(a))return `Use a tight close-up of ${product}. Put a finger or the motion directly on ${feature} so the viewer knows exactly where to look.`;if(/problem|contrast/.test(a))return `Open on the problem or awkward old motion for less than one second, then immediately switch to ${product} showing ${feature}.`;return `Open with ${product} already in frame and moving. Keep the first shot simple enough that the hook text and the product action make sense together.`;}
function ctaShot(cta,product){const style=clean(cta.style).toLowerCase();if(style==='demo-linked')return `End immediately after the successful demo motion. Hold ${product} cleanly in frame for about 1 second while the CTA lands.`;if(style==='qualifying')return `Finish on a clean, steady product shot. Do not add urgency; let the viewer decide while the CTA points to the listing.`;return `Finish on a clean product/result shot with ${product} unobstructed. Hold long enough to read the CTA.`;}
function makeTimeline(duration,mode,feature,hook,cta){const d=clampDuration(duration),hookEnd=Math.min(2,d*.16),demoEnd=Math.min(d-3,Math.max(hookEnd+3,d*.62)),benefitEnd=Math.min(d-1,Math.max(demoEnd+1,d*.84));return [{start:0,end:+hookEnd.toFixed(1),role:'HOOK',instruction:hook.shot},{start:+hookEnd.toFixed(1),end:+demoEnd.toFixed(1),role:'DEMO',instruction:`Show ${feature} clearly in use. Remove dead space and repeated setup.`},{start:+demoEnd.toFixed(1),end:+benefitEnd.toFixed(1),role:'BENEFIT',instruction:'Hold the clearest practical-use moment long enough to understand.'},{start:+benefitEnd.toFixed(1),end:d,role:'CLOSE',instruction:cta.shot}];}
...
function makeRecordingGuide(product,feature,hook,cta,packaging,duration){const hs=hookShot(hook.angle,product,feature),cs=ctaShot(cta,product),voice=makeVoiceover(hook,feature,cta,duration);return {durationSeconds:duration,hookAngle:hook.angle,hookText:hook.text,ctaStyle:cta.style,ctaText:cta.text,voiceover:voice,direction:`HOOK: ${hs}\nDEMO: Show ${feature} working clearly. Keep hands/product moving and keep your face out of frame.\nCTA: ${cs}`,shots:`0–2s HOOK — ${hs}\nMIDDLE DEMO — Show ${feature} clearly and remove repeated setup.\nFINAL SHOT — ${cs}`,onScreenText:`HOOK: ${hook.text}\nDEMO: ${feature.toUpperCase()}\nCTA: ${cta.text}`,sfx:'Whoosh on hook text in; light click/pop on feature text; snap/pop on CTA. Effects stay on a separate layer and never lower the original voice.',caption:packaging.caption,hashtags:packaging.hashtags.join(' '),cover:packaging.coverTitle,compliance:(packaging.compliance.flags.length?packaging.compliance.flags.join('; '):'No automatic wording flags. Use only verified product details.'),teleprompterPath:'teleprompter.html'};}
