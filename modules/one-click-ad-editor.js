/* DONE RITE Creator OS — One-Click Ad Editor engine v0.7 */
(function(){
'use strict';
const VERSION='0.7';
...
const TELEPROMPTER_HANDOFF_KEY='done-rite-one-click-teleprompter-handoff:v1';
...
H('Use case',[P[0],P[1]],(p,f)=>`Keep this where you reach for it during your regular routine. ${p}.`),
...
H('Reverse',[P[0],P[2]],(p,f)=>`If your current setup already covers this job, this may not be for you.`),
...
H('Probing',[P[0],P[3]],(p,f)=>`Before adding another accessory, look at this detail: ${f}.`),
...
{style:'Direct',platforms:[P[0]],text:'The orange cart has the full product details.'},
...
{style:'Qualifying',platforms:[P[0]],text:'If that matches your setup, the cart has the full listing.'},
...
const GENERIC_SCRIPT_WORDS=['amazing','obsessed','perfect','literally','need'];
function specificityFlags(value){const text=clean(value).toLowerCase();return GENERIC_SCRIPT_WORDS.filter(word=>new RegExp('\\b'+word+'\\b','i').test(text));}
...
function mainWinners(platform){try{const d=JSON.parse(localStorage.getItem(MAIN_OS_KEY)||'{}');return (Array.isArray(d.hookLog)?d.hookLog:[]).filter(x=>x&&x.winner&&(!x.platform||x.platform===platform)&&clean(x.text)&&!specificityFlags(x.text).length);}catch(e){return[];}}
...
function makeRecordingGuide(product,feature,hook,cta,packaging,duration){
  ...
  const stillImageReview=window.DoneRiteStillImageCompliance?window.DoneRiteStillImageCompliance.review({productName:product,feature,hook:hook.text,cta:cta.text,caption:packaging.caption,onScreenText:feature}):{status:'VISUAL_CONFIRMATION_REQUIRED',automaticFlags:[],teleprompterText:'Still-image visual confirmation is required before posting.'};
  ...
  return {durationSeconds:duration,hookAngle:hook.angle,hookText:hook.text,ctaStyle:cta.style,ctaText:cta.text,voiceover:voice,direction:plan,shots:[shot1,shot2,shot3,shot4].join('\n'),onScreenText:`HOOK: ${hook.text}\nDEMO: ${feature.toUpperCase()}\nCTA: ${cta.text}`,sfx:'Whoosh on hook text in; light click/pop on feature text; snap/pop on CTA. Effects stay on a separate layer and never lower the original voice.',caption:packaging.caption,hashtags:packaging.hashtags.join(' '),cover:packaging.coverTitle,compliance:(packaging.compliance.flags.length?packaging.compliance.flags.join('; '):'No automatic wording flags. Use only verified product details.'),stillImageReview,stillImageRules:stillImageReview.teleprompterText,teleprompterPath:'teleprompter.html',visualGuideType:r.kind};
}
function teleprompterUrl(plan){const g=plan&&plan.recordingGuide||{},payload={session:'1',product:plan.product||'',seconds:String(plan.durationSeconds||15),type:'full',tone:'confident',script:g.voiceover||'',direction:g.direction||'',angle:g.hookAngle||'',format:'Vertical 9:16 · '+(plan.platform||'TikTok Shop')+' · hands/product focused',shots:g.shots||'',onscreen:g.onScreenText||'',sfx:g.sfx||'',caption:g.caption||'',hashtags:g.hashtags||'',cover:g.cover||'',compliance:g.compliance||'',stillrules:g.stillImageRules||'',updatedAt:new Date().toISOString()};try{localStorage.setItem(TELEPROMPTER_HANDOFF_KEY,JSON.stringify(payload));}catch(e){}const q=new URLSearchParams();q.set('session','1');q.set('handoff','1');q.set('product',payload.product);q.set('seconds',payload.seconds);q.set('type','full');q.set('tone','confident');q.set('script',payload.script);return 'teleprompter.html?'+q.toString();}
...
window.DoneRiteOneClickAdEditor={version:VERSION,defaults:Object.assign({},DEFAULTS),createPlan,safeText,specificityFlags,genericScriptWords:GENERIC_SCRIPT_WORDS.slice(),categoryFlags,recordHookResult,getHookTestState:loadHookState,hookPool:HOOK_LIBRARY.slice(),ctaPool:CTA_LIBRARY.slice(),hookOptions,ctaOptions,teleprompterUrl,installRecordingHandoff};
})();
