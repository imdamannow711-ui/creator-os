...
function review(text,options){options=options||{};const body=clean(text),allowed=(Array.isArray(options.allowedBrandNames)?options.allowedBrandNames:[]).map(clean).map(value=>value.toLowerCase()),flags=[];Object.keys(GROUPS).forEach(group=>GROUPS[group].forEach(term=>{if(!hasTerm(body,term))return;if(group==='unrelatedBrands'&&allowed.includes(term.toLowerCase()))return;const message=LABELS[group]+': “'+term+'”';if(!flags.includes(message))flags.push(message);}));return {version:VERSION,status:flags.length?'FIX_BEFORE_RECORDING':'NO_DICTIONARY_FLAGS',flags};}
