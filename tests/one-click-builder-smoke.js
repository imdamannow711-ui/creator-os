...
const stillCompliance=fs.readFileSync(path.join(root,'modules','still-image-compliance.js'),'utf8');
const languageCompliance=fs.readFileSync(path.join(root,'modules','content-language-compliance.js'),'utf8');
...
new vm.Script(stillCompliance,{filename:'still-image-compliance.js'});
new vm.Script(languageCompliance,{filename:'content-language-compliance.js'});
...
assert(html.includes('modules/one-click-ad-editor.js?v=20260914-compliance-gate-1'),'Compliance-gate editor cache key was not refreshed');
assert(html.includes('modules/still-image-compliance.js?v=20260914-still-image-1'),'Still-image compliance module is not loaded');
assert(html.includes('modules/content-language-compliance.js?v=20260914-language-1'),'High-risk language dictionary is not loaded');
...
assert(html.includes('modules/one-click-ready-project.js?v=20260914-visual-compliance-1'),'Hollyland ready-project compliance cache key was not refreshed');
...
assert(editor.includes('stillImagePolicy:guide.stillImageReview'),'Still-image rules are not recorded in the edit plan');
assert(editor.includes('languagePolicy:languageReview'),'High-risk language review is not recorded in the edit plan');
assert(editor.includes("q.set('handoff','1')"),'Generic Teleprompter handoff is not using a short stored-session URL');
assert(html.includes('id="stillImageCompliance"'),'Still-image and realistic-results review is not visible before Teleprompter handoff');
assert(teleprompterBridge.includes('window.DoneRiteTeleprompterHandoff'),'Stored compliance handoff cannot reach the Teleprompter');
const teleprompter=fs.readFileSync(path.join(root,'teleprompter.html'),'utf8');
assert(teleprompter.includes('guideStillRulesBlock')&&teleprompter.includes("p.get('stillrules')"),'Teleprompter does not display the still-image rules');
...
assert(html.includes('styles/smoked-glass-theme.css?v=20260914-smoked-glass-1'),'Shared smoked-glass builder theme is not loaded');
assert(html.includes('modules/ui-touch-feedback.js?v=20260914-smoked-glass-1'),'Shared button touch feedback is not loaded');
...
assert(sw.includes('done-rite-v37-smoked-glass-touch'),'Service-worker cache was not refreshed for smoked-glass touch controls');
assert(sw.includes('modules/still-image-compliance.js')&&sw.includes('modules/content-language-compliance.js'),'Compliance modules are not available offline');
...
assert(readyProject.includes('stillrules'),'Hollyland Teleprompter handoff is missing still-image and realistic-result rules');
